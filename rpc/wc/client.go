...
func (client *WSClient) init() {
	...
	if client.ConnNum <= 0 {
		client.ConnNum = 1
		server.Warning("invalid ConnNum, reset to %v", client.ConnNum)
	}
	if client.ConnectInterval <= 0 {
		client.ConnectInterval = 3 * time.Second
		server.Warning("invalid ConnectInterval, reset to %v", client.ConnectInterval)
	}
	if client.PendingWriteNum <= 0 {
		client.PendingWriteNum = 100
		server.Warning("invalid PendingWriteNum, reset to %v", client.PendingWriteNum)
	}
	if client.MaxMsgLen <= 0 {
		client.MaxMsgLen = 4096
		server.Warning("invalid MaxMsgLen, reset to %v", client.MaxMsgLen)
	}
