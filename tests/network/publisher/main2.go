...
func PublishMessages(nc *nats.Conn) error {
	var (
		ev    Event
		bytes []byte
		err   error
	...
	for i := 0; i < 100; i++ {
		...
		if err = nc.Publish(StreamName+"."+LocationSubject, bytes); err != nil {
			fmt.Println("Error: " + err.Error())
			continue
		}
		...
		if err = nc.Publish(StreamName+"."+BatteryLevelSubject, bytes); err != nil {
			fmt.Println("Error: " + err.Error())
			continue
		}
...
func main() {
	...
	err = CreateStreamIfNotExists(StreamName, jc)
	if err != nil {
		fmt.Println(err.Error())
		os.Exit(1)
	}

	go PublishMessages(nc)

	// locSub, err := GetLocationsSubscription(jc)
	// if err != nil {
	// 	fmt.Println(err.Error())
	// 	os.Exit(1)
	// }
